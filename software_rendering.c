...
global_variable f32 scale_factor = 0.01f; // Hardcoded
...
internal void draw_rect_in_pixels(int x0, int y0, int x1, int y1, u32 color) {
	clamp(x0, 0, render_buffer.width);
	clamp(x1, 0, render_buffer.width);
	clamp(y0, 0, render_buffer.height);
	clamp(y1, 0, render_buffer.height);
...
internal void draw_rect(v2 p, v2 half_size, u32 color) {
	...
	half_size.x *= (f32)aspect_multiplier * scale_factor;
	half_size.y *= (f32)aspect_multiplier * scale_factor;

	p.x *= (f32)aspect_multiplier * scale_factor;
	p.y *= (f32)aspect_multiplier * scale_factor;
...
internal void clear_screen_and_draw_rect(v2 p, v2 half_size, u32 color, u32 clear_color) {
	f32 aspect_multiplier = calculate_aspect_multiplier();
